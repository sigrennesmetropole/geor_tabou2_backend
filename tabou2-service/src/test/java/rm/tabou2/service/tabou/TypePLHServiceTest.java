...
class TypePLHServiceTest extends DatabaseInitializerTest implements ExceptionTest {
	...
	void initTest() {
		...
		Mockito.when(programmeRightsHelper.checkCanUpdateProgramme(Mockito.any(), Mockito.anyBoolean()))
				.thenReturn(true);
	...
	void testCreateTypePLH() throws AppServiceException {
		...
		typePLH1.setDateDebut(Instant.now().minus(1, DAYS).atOffset(ZoneOffset.UTC));
		typePLH1.setDateFin(Instant.now().plus(1, DAYS).atOffset(ZoneOffset.UTC));
		...
		typePLH2.setDateDebut(Instant.now().minus(1, DAYS).atOffset(ZoneOffset.UTC));
		typePLH2.setDateFin(Instant.now().plus(1, DAYS).atOffset(ZoneOffset.UTC));
	...
	void testCreateTypePLHEnCascade() throws AppServiceException {
		...
		Assertions.assertEquals(TypePLH.TypeAttributPLHEnum.CATEGORY, plh1.getTypeAttributPLH());
		Assertions.assertEquals(1, plh1ApresFils.getFils().size());
		Assertions.assertEquals(typePLH2.getLibelle(),
				plhService.getTypePLH(plh1.getId()).getFils().get(0).getLibelle());
	}
	...
	void testGetTypePLHProgramme() throws AppServiceException {
		...
		pc.setDatDate(dateHelper.now());
		pc.setDocDate(dateHelper.now());
		...
		typePLHEntity1.setDateDebut(LocalDateTime.ofInstant(Instant.now().minus(1, DAYS), ZoneOffset.UTC));
		typePLHEntity1.setDateFin(LocalDateTime.ofInstant(Instant.now().plus(1, DAYS), ZoneOffset.UTC));
		...
		typePLHEntity2.setDateDebut(LocalDateTime.ofInstant(Instant.now().minus(2, DAYS), ZoneOffset.UTC));
		typePLHEntity2.setDateFin(LocalDateTime.ofInstant(Instant.now().minus(1, DAYS), ZoneOffset.UTC));
	...
	void testRemovePLHProgramme() throws AppServiceException {
		...
		Assertions.assertThrows(AppServiceException.class,
				() -> programmeService.getPLHProgramme(programmeId, plh1.getId()));
	}
