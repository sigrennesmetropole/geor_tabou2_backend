...
@Repository
public class TypeActeurCustomDaoImpl extends AbstractCustomDaoImpl<TypeActeurEntity> implements TypeActeurCustomDao {

    @PersistenceContext(unitName = "tabouPU")
    EntityManager entityManager;

    @Override
    public Page<TypeActeurEntity> searchTypesActeurs(TypeActeurCriteria criteria, Pageable pageable) {
        CriteriaBuilder builder = entityManager.getCriteriaBuilder();

        //Requête pour compter le nombre de résultats total
        CriteriaQuery<Long> countQuery = builder.createQuery(Long.class);
        Root<TypeActeurEntity> countRoot = countQuery.from(TypeActeurEntity.class);
        ...
        CriteriaQuery<TypeActeurEntity> searchQuery = builder.createQuery(TypeActeurEntity.class);
        Root<TypeActeurEntity> searchRoot = searchQuery.from(TypeActeurEntity.class);
        buildQuery(criteria, builder, searchQuery, searchRoot);
        searchQuery.orderBy(QueryUtils.toOrders(pageable.getSort(), searchRoot, builder));

        TypedQuery<TypeActeurEntity> typedQuery = entityManager.createQuery(searchQuery);
        List<TypeActeurEntity> tiersEntities = typedQuery.setFirstResult((int) pageable.getOffset()).setMaxResults(pageable.getPageSize()).getResultList();
        return new PageImpl<>(tiersEntities, pageable, totalCount.intValue());
    }

    private void buildQuery(TypeActeurCriteria criteria, CriteriaBuilder builder,
                            CriteriaQuery<?> query, Root<TypeActeurEntity> root) {
